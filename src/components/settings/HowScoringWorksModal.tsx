...
type HowScoringWorksModalProps = {
  isOpen: boolean;
  onClose: () => void;
};

export const HowScoringWorksModal = ({ isOpen, onClose }: HowScoringWorksModalProps) => {
  const t = useTranslations("settings.quickActions.howScoringWorksModal");
  ...
  const handleGoToScoringModel = () => {
    router.replace(`${pathname}?tab=scoringModel`);
    onClose();
  };
  ...
  const content = (
    <Shell>
        ...
        <Header>
          <HeaderStart>
            ...
            <TitleGroup>
              ...
              <Description id={descriptionId}>{t("intro")}</Description>
            </TitleGroup>
          </HeaderStart>
        ...
        <FactorList>
          {scoringFactorKeys.map((factorKey) => (
            <FactorItem key={factorKey}>
              ...
              <FactorDescription>{tFactors(`${factorKey}.description`)}</FactorDescription>
            </FactorItem>
          ))}
        </FactorList>

        <CustomizeHint>{t("customizeHint")}</CustomizeHint>
      </Body>
      ...
      <Footer>
        ...
        <Button onClick={handleGoToScoringModel}>{t("goToScoringModel")}</Button>
      </Footer>
    </Shell>
  );
