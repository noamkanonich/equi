...
const safetyRuleKeys = [
  "noGuaranteedReturns",
  "noTrades",
  "decisionSupportOnly",
  "reviewRisks",
  "missingDataLowersConfidence",
] as const;

type AiSafetyRulesModalProps = {
  isOpen: boolean;
  onClose: () => void;
};

export const AiSafetyRulesModal = ({ isOpen, onClose }: AiSafetyRulesModalProps) => {
  const t = useTranslations("settings.aiPreferences.modals");
  const tRules = useTranslations("settings.aiPreferences.safetyRules");
  ...
  const content = (
    <Shell>
      {!isMobile ? (
        <Header>
          <HeaderStart>
            ...
            <TitleGroup>
              <Title id={titleId}>{t("safetyRulesTitle")}</Title>
              <Description id={descriptionId}>{tRules("decisionSupportOnly")}</Description>
            </TitleGroup>
          </HeaderStart>
      ...
      <Body>
        <RuleList>
          {safetyRuleKeys.map((key) => (
            <RuleItem key={key}>{tRules(key)}</RuleItem>
          ))}
        </RuleList>
      </Body>
