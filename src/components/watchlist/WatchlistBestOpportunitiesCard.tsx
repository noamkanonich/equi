...
type WatchlistBestOpportunitiesCardProps = {
  stocks: WatchlistSidebarStock[];
  locale: string;
};

export const WatchlistBestOpportunitiesCard = ({
  stocks,
  locale,
}: WatchlistBestOpportunitiesCardProps) => {
  ...
  return (
    ...
    <Card>
      <TitleRow>
        <Trophy size={16} strokeWidth={1.8} aria-hidden />
        <Title>{t("sidebar.bestOpportunities")}</Title>
      </TitleRow>
      <List>
        {stocks.map((stock, index) => (
          <ListItem key={stock.symbol}>
            ...
            <Score>{stock.value}</Score>
          </ListItem>
        ))}
      </List>
      <CardLink type="button" onClick={() => setIsOpen(true)}>
        {t("actions.viewFullRanking")}
    ...
    <PlaceholderModal
      ...
      title={tInteractions("features.rankingTitle")}
      description={tInteractions("features.rankingDescription")}
    />
    </>
  );
};
